...
maxInt :: Tree -> Int
maxInt (Leaf i)     = i
maxInt (Node l m n) = max l (max (maxInt m) (maxInt n))
...
minInt :: Tree -> Int
minInt (Leaf i)     = i
minInt (Node l m n) = min l (min (minInt m) (minInt n))
...
sumInts :: Tree -> Int
sumInts (Leaf i)    = i
sumInts (Node l m n) = l + sumInts(m) + sumInts(n)
...
preorder :: Tree -> [Int]
preorder (Leaf i)   = [i]
preorder (Node l m n) = [l] ++ preorder m ++ preorder n
...
inorder :: Tree -> [Int]
inorder (Leaf i)    = [i]
inorder (Node l m n) = inorder m ++ [l] ++ inorder n
...
isBST :: Tree -> Bool
isBST (Leaf i)   = True
isBST (Node l m n) = bstVal m < l && l < bstVal n
...
inBST :: Int -> Tree -> Bool
inBST k (Leaf i) = k == i
inBST k (Node l m n) = k == l || inBST k m ||  inBST k n
